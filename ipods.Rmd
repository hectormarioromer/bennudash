...
Para calcular la columna *health* establecemos el valor 3 como el requerido para operación en tienda. Dividimos la condición promedio de los equipos entre la condición requerida y lo multiplicamos por 100.
...
Actualmente los `r h$total` IPODs desplegados tienen una condición promedio de **`r h$avg_condition`**, lo que representa una salud de **`r h$health`** para el parque operativo.


## ¿Cuántos IPODs han ido a taller? 
#### ¿cuántos fueron reparados? ¿cuántos fueron descartados?

A la fecha hemos llevado **`r nrow(mant)`** equipos a taller. Como resultado, **`r reparados`** equipos fueron *reparados* y reintegrados al parque, para un porcentaje de **`r round(reparados/nrow(mant)*100, digits = 2)`%**, mientras que **`r descartados`** fueron *descartados*, para un porcentaje de **`r round(descartados/nrow(mant)*100, digits = 2)`%**. 
